   ...
   handleAdd(id)
   {
    ...
    render() {
        return (
            <div>
                <div> 
                <Button style={{margin:15}} onClick={()=>{this.setState({learnt:1})}} variant="contained" color="primary">
                    Show Words      </Button>
                <Button style={{margin:15}} onClick={()=>{this.setState({learnt:0})}} variant="contained" color="primary">
                    Learnt words      </Button>
                </div>
                {
                    this.state.learnt ?
                    (<DisplayTable userid={this.props.id} data={this.state.all} type="all" onAdd={(id)=>this.handleAdd(id)}/>):
                    (<DisplayTable userid={this.props.id} data={this.state.custom} type="custom" onDelete={(id)=>this.handleDelete(id)}/>)
                }
            </div>
        );
    }
